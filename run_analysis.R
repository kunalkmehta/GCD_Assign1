run_analysis<-function(){
  ...
  TrainxData<-read.table(filextrain)
    
  filextest<-"UCI HAR Dataset/test/X_test.txt"
  TestxData<-read.table(filextest)
  
  totalx<-rbind(TrainxData, TestxData)
  ...
  TrainyData<-read.table(fileytrain)
   
  fileytest<-"UCI HAR Dataset/test/y_test.txt"
  TestyData<-read.table(fileytest)
   
  totaly<-rbind(TrainyData, TestyData)
  names(totaly)<-"Activity";
  ######## End Merging X_Train and X_Test ########
  ...
  names(totalx)<- FeatureData; ## Give descriptive column names

  totalselectx<- totalx[,checkgrep] ## Subsetting total X data for mean and std measurements

  
  ###### Creating Complete Data set by merging all data sets prepared earlier ######  
    completeset<-cbind(totalselectx, totalsubject, totaly) 
